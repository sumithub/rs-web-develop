      ...
      <div className="relative">
        ...
        >
          ...
          <div className="absolute top-1/2 transform -translate-y-1/2 w-full h-2 bg-[#F6F8FB] rounded-full">
            {/* Active Range */}
            <div 
              className="h-full bg-blue-500 rounded-full absolute transition-all duration-150"
              style={{
          ...
          {/* Min Handle */}
          <div
            className={`absolute top-1/2 transform -translate-y-1/2 -translate-x-1/2 w-6 h-6 bg-white border-2 border-border-color rounded-full cursor-grab transition-transform duration-150 ${
        ...
        <div className="flex justify-between items-center mt-4 px-2">
          {stars.map((star) => (
            <div key={star} className="flex gap-1 items-center">
              ...
              <span className="text-sm text-gray-600 mt-1 select-none">{star}</span>
            </div>
          ))}
        </div>
