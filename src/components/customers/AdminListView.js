...
import { adminCustomerHistory } from "../../constent/constArray"
...
export default function AdminListView() {
    ...
    const getHistory = async () => {
        try {
            ...
            setList(res.data || adminCustomerHistory)
            setLoading(false)
            ...
            {loading ? <Loading /> : (list?.length > 0 ? <table className='w-full'>
                <thead>
                    <tr>
                        ...
                        <th>
                            <div className="flex justify-center">
                                <TableOrder title="Total Customers"
                                    sortBy={sortBy}
                                    setSortBy={setSortBy}
                ...
                <tbody>
                    {list?.map((e, index) => <tr key={index} className={index === list.length - 1 ? '' : 'border-b border-border-color'}>
                        ...
                        <td><div className="flex justify-center">{formatDate(e.createdOn)}</div></td>
                        <td><div className="flex justify-center">{e.client}</div></td>
