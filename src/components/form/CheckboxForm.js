...
export default function CheckboxForm({
  ...
  return (
    <div>
      <label
        className={`${class_} ${disabled ? "pointer-events-none" : ""}`}
        htmlFor={id || label}
      >
        <div className="pointer-events-none">
          <div className={`text-base text-secondary ${labelClass}`}>
            {label}
          </div>
        ...
        <input
          ...
          disabled={disabled}
          type="checkbox"
          className={`h-4 w-5 ml-auto mt-1 ${inputClass}`}
          {...formProps}
          onChange={(e) => {
      ...
      {error && (
        <p className="text-xs pt-[5px] capitalize text-danger">{error}</p>
      )}
    </div>
  );
}
