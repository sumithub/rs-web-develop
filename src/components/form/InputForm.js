...
export default function InputForm({
  ...
  return (
    <div className={`mt-[15px] ${class_}`}>
      <label
        className={`text-sm font-medium text-secondary inline-flex items-center gap-[5px] capitalize ${labelClass}`}
      >
        {" "}
        {infoIcon && (
      ...
      <div className="relative">
        ...
        {inputType === "password" && (
          <Image
            unoptimized
            src={
              type !== "password" ? "/images/eyes.svg" : "/images/open-eye.svg"
            }
            alt="icon"
            width={16}
            height={16}
        ...
        {isTextArea ? (
          <textarea
            ...
            } focus:outline-0 focus-visible:outline-0 focus:border-primary/60 w-full rounded-lg py-3 px-2.5 text-sm text-secondary ${inputClass}`}
          ...
          <input
            ...
            type={inputType === "password" ? type : inputType || "text"}
            {...formProps}
            disabled={disabled}
            className={`border ${
              error ? "border-danger" : "border-primary/10"
      ...
      {error && (
        <p className="text-xs pt-[3px] capitalize text-danger">{error}</p>
      )}
    </div>
  );
}
